...
STOCK_REPORT_CACHE_DIR = CACHE_DIR / "stock_reports"
VIDEO_OPINIONS_CACHE_DIR = CACHE_DIR / "video_opinions"
...
# 수집 시점 다중 종목 의견 추출 (영상당 LLM 1회, 검색 시 LLM 제거)
INGEST_OPINION_EXTRACTION_ENABLED = os.environ.get("INGEST_OPINION_EXTRACTION_ENABLED", "1") == "1"
VIDEO_OPINIONS_CONTEXT_MAX_CHARS = int(os.environ.get("VIDEO_OPINIONS_CONTEXT_MAX_CHARS", "12000"))
...
POPULAR_PREWARM_MARKET_LIMIT = int(os.environ.get("POPULAR_PREWARM_MARKET_LIMIT", "30"))
SEARCH_RESULT_PREWARM_ENABLED = os.environ.get("SEARCH_RESULT_PREWARM_ENABLED", "1") == "1"
SEARCH_RESULT_PREWARM_LIMIT = int(os.environ.get("SEARCH_RESULT_PREWARM_LIMIT", "5"))
SEARCH_RESULT_PREWARM_DELAY_SECONDS = float(os.environ.get("SEARCH_RESULT_PREWARM_DELAY_SECONDS", "0.5"))
WATCHLIST_REFRESH_RATE_LIMIT_WINDOW_SECONDS = int(os.environ.get("WATCHLIST_REFRESH_RATE_LIMIT_WINDOW_SECONDS", "60"))
WATCHLIST_REFRESH_RATE_LIMIT_MAX = int(os.environ.get("WATCHLIST_REFRESH_RATE_LIMIT_MAX", "5"))
WATCHLIST_REFRESH_RESPONSE_CACHE_SECONDS = int(os.environ.get("WATCHLIST_REFRESH_RESPONSE_CACHE_SECONDS", "30"))
...
# source_type : 출처 성격 — "creator" | "news"
#        ※ 뉴스/경제TV 채널은 사실·시황 보강 신뢰는 높지만 전망 신호 가중치는 낮게 본다.
# 채널 성향은 최근 영상 기준으로 계속 보정해야 하는 보조 메타데이터.
def _channel(name, lean, kind, categories, channel_id="", tracked=False, source_type="creator"):
    return {
        "name": name,
...
CHANNELS = [
    ...
    _channel("한국경제TV뉴스", "중립", "시황", ["국내주식", "거시시황"], source_type="news"),
    ...
    _channel("토마토증권통", "중립", "종목", ["국내주식", "거시시황"], source_type="news"),
    _channel("매일경제TV", "중립", "시황", ["국내주식", "거시시황"], source_type="news"),
    _channel("MTN 머니투데이방송", "중립", "종목", ["국내주식", "거시시황"], source_type="news"),
    _channel("서울경제TV", "중립", "시황", ["국내주식", "거시시황"], source_type="news"),
    _channel("이데일리TV", "중립", "시황", ["국내주식", "거시시황"], source_type="news"),
    _channel("연합뉴스경제TV", "중립", "시황", ["국내주식", "거시시황"], source_type="news"),
    _channel("한국경제TV", "중립", "시황", ["국내주식", "거시시황"], source_type="news"),
    _channel("매경 월가월부", "중립", "시황", ["미국주식", "거시시황"]),
    _channel("어썸머니", "중립", "시황", ["국내주식", "미국주식", "거시시황"]),
    _channel("한경 글로벌마켓", "중립", "시황", ["미국주식", "거시시황"], source_type="news"),
    ...
    # 리포트 표본 보강: 증권사·리서치 채널과 종목/섹터 특화 채널.
    _channel("채널K by 키움증권", "중립", "종목", ["국내주식", "미국주식", "반도체", "거시시황"]),
    _channel("채널K 글로벌 by 키움증권", "중립", "종목", ["미국주식", "거시시황"]),
    _channel("미래에셋 스마트머니", "중립", "시황", ["국내주식", "미국주식", "거시시황"]),
    _channel("NH투자증권", "중립", "시황", ["국내주식", "미국주식", "거시시황"]),
    _channel("투자로그인 by NH투자증권", "중립", "종목", ["국내주식", "미국주식", "거시시황"]),
    _channel("깨비증권 마블TV", "중립", "종목", ["국내주식", "미국주식", "거시시황"]),
    _channel("알파TV - 신한투자증권", "중립", "종목", ["국내주식", "미국주식", "반도체", "거시시황"]),
    _channel("하나TV", "중립", "시황", ["국내주식", "미국주식", "거시시황"]),
    _channel("Daishin TV", "중립", "시황", ["국내주식", "미국주식", "거시시황"]),
    _channel("유안타증권", "중립", "종목", ["국내주식", "미국주식", "거시시황"]),
    _channel("한국투자증권", "중립", "시황", ["국내주식", "미국주식", "거시시황"]),
    _channel("한경 코리아마켓", "중립", "종목", ["국내주식", "거시시황"], source_type="news"),
    _channel("MBN골드", "중립", "종목", ["국내주식", "거시시황"], source_type="news"),
    _channel("뉴스핌TV", "중립", "시황", ["국내주식", "거시시황"], source_type="news"),
    _channel("제도권주식분석 by ETF STUDIO", "중립", "종목", ["국내주식", "미국주식", "반도체"]),
    _channel("탑건 : 반도체 엔지니어의 주식 이야기", "중립", "종목", ["반도체", "국내주식", "미국주식"]),
]
